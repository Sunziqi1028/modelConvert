...
class ModelPanel extends React.Component {
    ...
    handleClick(model) {
        ...
        loader.load(url, model, {
            ...
            obj.name = model.Name;
            global.app.ModelID =  model.ID;
            Object.assign(obj.userData, model, {
                Server: true
            });
            console.log('size2------->', this.getObjectHalfSize(obj));
            global.app.model_size = this.getObjectHalfSize(obj);
    ...
    getObjectHalfSize(obj) {
        var objectBox = new THREE.Box3();
        objectBox.setFromObject(obj);
        return objectBox.max.clone().sub(objectBox.min).divideScalar(2).multiplyScalar(1000);
    }
    // 添加到场景中心
    addToCenter(obj) {
        global.app.editor.clear()
        var cmd = new AddObjectCommand(obj);
        cmd.execute();
        global.app.objuuid = obj.uuid;
        obj.scale.set(0.1, 0.1, 0.1);
        global.app.editor.focus(obj);
    }
