...
class CharacterPanel extends React.Component {
    ...
    update() {
        fetch(`${global.app.options.server}/api/Category/List?Type=Character`).then(response => {
        ...
        fetch(`${global.app.options.server}/api/Character/List`).then(response => {
            response.json().then(obj => {
                if (obj.Code !== 200) {
                    global.app.toast(_t(obj.Msg), 'warn');
                    return;
    ...
    handleClick(data) {
        Ajax.get(`${global.app.options.server}/api/Character/Get?ID=${data.ID}`, result => {
    ...
    handleEdit(data) {
        var win = global.app.createElement(EditWindow, {
            ...
            data,
            saveUrl: `${global.app.options.server}/api/Character/Edit`,
    ...
    handleDelete(data) {
        global.app.confirm({
            ...
            onOK: () => {
                fetch(`${global.app.options.server}/api/Character/Delete?ID=${data.id}`, {
                    method: 'POST'
                }).then(response => {
                    response.json().then(obj => {
                        if (obj.Code === 200) {
