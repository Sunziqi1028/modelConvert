...
import SetMaterialUserDataCommand from '../../command/SetMaterialUserDataCommand';
...
class MaterialComponent extends React.Component {
    ...
    render() {
        ...
        >
            <SelectProperty label={'材质通道类型'}
                            options={custom_materials}
                            name={'custom_type'}
                            value={custom_type}
    ...
    handleMaterial(material) {
        ...
        let state = {
            show: true,
            type: material.type,
            custom_type: global.app.custom_type,
            light_intensity: global.app.light_intensity,
        ...
        if (this.material.custom_type !== undefined) {
            state.custom_type = material.custom_type;
            state.custom_material_value = custom_materials[material.custom_type];
        }
        state.light_intensity = 0;
        if (this.material.light_intensity !== undefined) {
            state.light_intensity = material.light_intensity;
        }
        this.setState(state);
    }
    ...
    handleChange(value, name) {
        ...
        if (name === 'custom_type') {
            global.app.set_material = true;
            console.log(material.userData, custom_type, custom_materials, custom_materials[custom_type]);
            editor.execute(new SetMaterialValueCommand(object, 'custom_type', custom_type));
            editor.execute(new SetMaterialValueCommand(object, 'custom_material_value', custom_materials[custom_type]));
            console.log('custom_type', object)
        }

        if (name === 'light_intensity') {
            editor.execute(new SetMaterialValueCommand(object, 'light_intensity', light_intensity));
        }
    ...
    handleCustomTypeChange(value) {
        console.log('material:', this.selected.material);
        const {
            custom_materials,
        } = Object.assign({}, this.state, {
            [name]: value
        ...
        if (Array.isArray(this.selected.material)) {
            this.selected.material.userData[this.materialIndex].custom_type = value;
            this.selected.material.userData[this.materialIndex].custom_material_value = custom_materials[value];
        } else {
            this.selected.material.userData.custom_type = value;
            this.selected.material.userData.custom_material_value = custom_materials[value];
        }
        console.log('material:', this.selected.material);
        this.setState({
            custom_type: value
